...
@interface MinePublishDetailController ()<UITableViewDataSource,
UITableViewDelegate,
ParnterFavCommentShareViewDelegate,
...
@property (nonatomic, strong) ParnterFavCommentShareView * bottomView;
...
@implementation MinePublishDetailController
...
//    [seeRuleBtn setImage:[UIImage imageNamed:@"yb_cd"] forState:UIControlStateNormal];
//    [seeRuleBtn setImage:[UIImage imageNamed:@"yb_cd"] forState:UIControlStateSelected];
//    [seeRuleBtn addTarget:self action:@selector(onRightBtn:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)updateData{
    ...
    ParnterFavCommentShareView * bottomView = [[ParnterFavCommentShareView alloc]init];
    
    if (kDevice_Is_iPhoneX) {
        bottomView.frame = CGRectMake(0,
                                      SCREEN_HEIGHT - 64 - 50 - 20,
                                      SCREEN_WIDTH,
                                      50);
    }else{
        bottomView.frame = CGRectMake(0,
    ...
    GetPartnerDetailHeader * headerView = [[GetPartnerDetailHeader alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 480 + 44 + 34)];
...
- (void)bottomViewOperation:(KBottomOpreationType)type{
    /**赞*/
    if(KBottomCollection == type){
        [self userFavGetTheParnter];
    }
    /** 评论*/
    if(KBottomComment == type){
...
//    popView.iconNameArray = @[@"yb_dz",@"yb_dz"];
//    popView.itemNameArray = @[@"分享",@"发布"];
//    popView.itemH     = itemH;
