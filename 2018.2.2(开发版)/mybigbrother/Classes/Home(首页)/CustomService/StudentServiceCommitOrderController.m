//
//  StudentServiceCommitOrderController.m
//  mybigbrother
//
...
@interface StudentServiceCommitOrderController()<UITableViewDelegate,
...
@implementation StudentServiceCommitOrderController
...
- (void)rihgtButtonClicked{
    for (int i = 0 ; i < _leftDatas.count; i ++ ) {
        ...
        if (i  == 9) {
            ...
            self.infoDic[@"stage"] = content;
        }
        /** 开始时间*/
        if (i  == 10) {
            self.infoDic[@"start_time"] = content;
            
        }
        /** 结束时间*/
        if (i  == 11) {
            self.infoDic[@"end_time"] = content;
            
        }
        /** 备注个性需求*/
        if (i  == 12) {
            self.infoDic[@"desc"] = content;
...
#pragma mark  - || 之后字段与_leftDatas 中原始数据相匹配
    ...
    if ([self.infoDic[@"city"]isEqualToString:@""]||[self.infoDic[@"city"] isEqualToString:@"请输入"]) {
        [MyControl alertShow:@"请您输入就读城市"];
        return;
    }
    if ([self.infoDic[@"school"]isEqualToString:@""]||[self.infoDic[@"school"] isEqualToString:@"请输入"]) {
        [MyControl alertShow:@"请您输入就读学校"];
        return;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 10){
        ...
        picker.delegate = self;
        picker.tag = KDatePickerBeginDay;
        [self.view addSubview:picker];
    }
    if (indexPath.row == 11){
        /** 选择结束日期*/
...
//        
//    }
//    if (KDatePickerBeginDay == picker.tag) {
//        OrderInfoCell * cell = [self.menuTableView cellForRowAtIndexPath:self.currentIndexPath];
//        cell.rightField.text = dateStr;
//        self.infoDic[@"start_time"] = dateStr;
//    }
//    if (KDatePickerEndDay == picker.tag) {
//        OrderInfoCell * cell = [self.menuTableView cellForRowAtIndexPath:self.currentIndexPath];
//        cell.rightField.text = dateStr;
//        self.infoDic[@"end_time"] = dateStr;
//    }
    OrderInfoCell * cell = [self.menuTableView cellForRowAtIndexPath:self.currentIndexPath];
    cell.rightField.text = dateStr;
