...
//    self.selectedImageview.frame = CGRectMake([UIScreen mainScreen].bounds.size.width/2, (self.height -16)/2, 16, 16);
    self.title.frame = CGRectMake([UIScreen mainScreen].bounds.size.width/2 - 30, 0, 100, self.height);
    if (_item.subTitle != nil) {
        self.subTitle.frame = CGRectMake([UIScreen mainScreen].bounds.size.width/2-30 , 0, 100, self.height);
    }
