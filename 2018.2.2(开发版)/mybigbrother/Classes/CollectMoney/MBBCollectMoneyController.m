...
@interface MBBCollectMoneyController ()<CustomServiceDetailBottomViewDelegate,MBBChoosePayMethodViewDelegate>
/** 付款方式*/
...
/** 付款方式选择*/
@property (nonatomic, strong) MBBChoosePayMethodView * payChooseView;

/** 底部支付view*/
@property (nonatomic, strong) CustomServiceDetailBottomView * bottomView;
@end

@implementation MBBCollectMoneyController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"收银台";
    self.view.backgroundColor = [UIColor whiteColor];
    MBBChoosePayMethodView * payChooseView;
    
    if (kDevice_Is_iPhoneX) {
        payChooseView = [[MBBChoosePayMethodView alloc]initWithFrame:CGRectMake(0,84, SCREEN_WIDTH, 132)];
    }else{
        ...
        for (UIViewController *controller in detailVC.navigationController.viewControllers) {
            ...
            if ([controller isKindOfClass:[MBBHomeViewController class]]) {
               payChooseView = [[MBBChoosePayMethodView alloc]initWithFrame:CGRectMake(0,64, SCREEN_WIDTH, 132)];
//                return;
            }else if ([controller isKindOfClass:[MBBServiceViewController class]]){
                /** 服务 (返回服务一级)*/
                payChooseView = [[MBBChoosePayMethodView alloc]initWithFrame:CGRectMake(0,64, SCREEN_WIDTH, 132)];
//                 return;
            }else if ([controller isKindOfClass:[MyOrderController class]]){
                 /** 订单列表 (返回订单列表)*/
                payChooseView = [[MBBChoosePayMethodView alloc]initWithFrame:CGRectMake(0,0, SCREEN_WIDTH, 132)];
            }else{
