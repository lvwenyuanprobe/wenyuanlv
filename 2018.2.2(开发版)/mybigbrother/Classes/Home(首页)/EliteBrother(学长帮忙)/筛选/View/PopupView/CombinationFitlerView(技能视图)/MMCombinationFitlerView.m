...
- (void)popupViewFromSourceFrame:(CGRect)frame completion:(void (^ __nullable)(void))completion {
    ...
    [UIView animateWithDuration:AnimationDuration animations:^{
        ...
        self.bottomView.frame = CGRectMake(0, self.mainTableView.bottom, self.width, PopupViewTabBarHeight+20);
        [self addSubview:self.bottomView];
        NSArray *titleArray = @[@"重置",@"确定"];
        for (int i = 0; i < 2 ; i++) {
            CGFloat left = ((i == 0)?ButtonHorizontalMargin:self.width - ButtonHorizontalMargin - 165);
            
            UIColor *titleColor = ((i == 0)?[UIColor colorWithHexString:titleSelectedColor]:[UIColor whiteColor]);
            UIColor *backgroundColor = ((i == 0)?[UIColor colorWithHexString:@"FCFAFD"]:[UIColor colorWithHexString:titleSelectedColor]);
            UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
            button.frame = CGRectMake(left, 10, 165, PopupViewTabBarHeight);
            button.tag = i;
            ...
            button.layer.borderColor = RGB(247, 97, 58).CGColor;
